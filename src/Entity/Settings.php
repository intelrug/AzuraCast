...
class Settings
{
    ...
    /**
     * @OA\Property(example=SAMPLE_TIMESTAMP)
     * @var int The UNIX timestamp when the "manual" sync task was last run.
     *
     * @AuditLog\AuditIgnore
     */
    protected int $syncManualLastRun = 0;

    public function getSyncManualLastRun(): int
    {
        return $this->syncManualLastRun;
    }

    public function setSyncManualLastRun(int $syncManualLastRun): void
    {
        $this->syncManualLastRun = $syncManualLastRun;
    }
    ...
    public function getSyncLastRunTime(string $type): int
    {
        $timesByType = [
            ...
            GetSyncTasks::SYNC_MANUAL => $this->syncManualLastRun,
        ];
    ...
    public function updateSyncLastRunTime(string $type): void
    {
        switch ($type) {
            ...
            case GetSyncTasks::SYNC_MANUAL:
                $this->syncManualLastRun = time();
                break;
        }
    }
